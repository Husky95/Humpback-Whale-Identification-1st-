...
def try_bestfitting_loss(results, labels, selected_num=10):
    ...
    one_hot_target = torch.zeros(batch_size, class_num ).cuda().scatter_(1, labels, 1)[:, :15587].contiguous()
...
def sigmoid_loss(results, labels, topk=10):
    ...
    batch_size, class_num = results.shape
    labels = labels.view(-1, 1)
    one_hot_target = torch.zeros(batch_size, class_num ).cuda().scatter_(1, labels, 1)[:, :15587]
