...
def try_bestfitting_loss(results, labels, selected_num=10):
    ...
    one_hot_target = torch.zeros(batch_size, class_num + 1).cuda().scatter_(1, labels, 1)[:, :15586].contiguous()
    error_loss = lovasz_hinge(results, one_hot_target)
    labels = labels.view(-1)
    indexs_new = (labels != 15586).nonzero().view(-1)
    if len(indexs_new) == 0:
        return error_loss
...
def sigmoid_loss(results, labels, topk=10):
    ...
    batch_size, class_num = results.shape
    labels = labels.view(-1, 1)
    one_hot_target = torch.zeros(batch_size, class_num + 1).cuda().scatter_(1, labels, 1)[:, :15586 * 2]
    ...
    labels = labels.view(-1)
    indexs_new = (labels != 15586 * 2).nonzero().view(-1)
    if len(indexs_new) == 0:
        return error_loss
    results_nonew = results[torch.arange(0, len(results))[indexs_new], labels[indexs_new]].contiguous()
    target_nonew = torch.ones_like(results_nonew).float().cuda()
...
if __name__ == '__main__':
    results = torch.randn((4, 15586)).cuda()
    targets = torch.from_numpy(np.array([1,2,3,15586])).cuda()
