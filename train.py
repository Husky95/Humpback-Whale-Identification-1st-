...
def train(freeze=False, fold_index=1, model_name='seresnext50',min_num_class=10, checkPoint_start=0, lr=3e-4, batch_size=36):
    ...
    # Image,Id
    data_train = pd.read_csv('/kaggle/input/happy-whale-and-dolphin/train.csv'.format(fold_index))
    names_train = data_train['image'].tolist()
    labels_train = data_train['individual_id'].tolist()
    data_valid = pd.read_csv('/kaggle/input/happy-whale-and-dolphin/train.csv'.format(fold_index))
    names_valid = data_valid['image'].tolist()
    labels_valid = data_valid['individual_id'].tolist()
